...
function M.parse(src)
  local root = make_node("col")
  local stack = { root }
  local i, iter = 1, 0

  while i <= #src do
    iter = iter + 1
    if iter % 2000 == 0 then sleep(0) end

    if src:sub(i, i + 3) == "<!--" then
      local ci = src:find("%-%->", i + 4) or (#src - 2)
      i = ci + 3
    else
      local si, ei, closing, tag, attr = src:find("<%s*(/?)%s*([%w]+)([^>]*)>", i)
      if si == i and tag then
        local is_close = closing == "/"
        local self_close = attr:match("/%s*$") ~= nil
        i = ei + 1
        if is_close then
          ...
          if tag == "text" then
            local _, tei, inner = src:find("([%z\1-\255]-)</%s*text%s*>", i)
            node.attrs._text = inner or ""
            i = (tei and tei + 1) or i

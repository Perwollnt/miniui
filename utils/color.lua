...
function M.parse_color(name)
  ...
  for k, _ in pairs(M.COLOR) do
    if k:lower() == lower then return M.COLOR[k] end
  end

  local num = tonumber(v)
  if num and num >= 0 and num <= 15 then
    return M.COLOR[ORDER[num + 1]]
  end

  local palette = {
    white = { 1, 1, 1 }, orange = { 1, 0.5, 0 }, magenta = { 1, 0, 1 }, lightBlue = { 0.5, 0.75, 1 },
    yellow = { 1, 1, 0 }, lime = { 0.5, 1, 0.5 }, pink = { 1, 0.5, 0.5 }, gray = { 0.3, 0.3, 0.3 },
    lightGray = { 0.6, 0.6, 0.6 }, cyan = { 0, 1, 1 }, purple = { 0.5, 0, 0.5 }, blue = { 0, 0, 1 },
    brown = { 0.6, 0.4, 0.2 }, green = { 0, 1, 0 }, red = { 1, 0, 0 }, black = { 0, 0, 0 },
  }

  local r, g, b = hex_to_rgb(v)
  if not r then r, g, b = rgb_fn_to_rgb(v) end
  if not r then return nil end

  local best, best_name = 1e9, "white"
  for n, rgb in pairs(palette) do

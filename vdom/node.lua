...
function Node.new(tag, props, children)
  ...
  local s = {}
  if def and def.defaults then
    for k, v in pairs(def.defaults) do s[k] = v end
  end
...
local function compute_styles(node, parent)
  ...
  if parent then
    for k, _ in pairs(INHERIT) do
      if base[k] == nil and parent.computed and parent.computed[k] then base[k] = parent.computed[k] end
    end
  end
  node.computed = base

...
local styles = require("markup.style")
...
function M.measure(node, availW, availH)
  ...
  if node.tag == "text" then
    ...
    local lines = wrap_text(strings.split_words, node.attrs._text or "", w)
  ...
  elseif node.tag == "row" then
    ...
    for i, c in ipairs(kids) do
      local ch = targets[i]
      M.measure(c, innerW, ch)
      local usedH = math.max(ch, c.layout.outerH)

      c.layout.x = 0
      c.layout.y = y
      c.layout.w = c.layout.outerW
      c.layout.h = usedH
...
function M.layout_tree(root, screenW, screenH)
  Node.compute_styles(root, nil)
  M.measure(root, screenW, screenH)
